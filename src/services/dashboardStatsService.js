...
export const dashboardStatsService = {
  ...
  getZonalAdminDashboardStats: async () => {
    try {
      ...
      // Get pickup station stats for the zonal admin's zone
      let pickupStationStats = { total: 0 };
      if (userProfile?.zone?._id) {
        try {
          const pickupStatsResponse = await api.get(`/api/pickup-stations/by-zone/${userProfile.zone._id}/stats`);
          const responseData = pickupStatsResponse.data?.data || pickupStatsResponse.data || {};
          pickupStationStats = responseData;
        } catch (error) {
          console.warn('DashboardStats: Failed to fetch pickup station stats:', error);
        }
      }
      ...
      const stats = {
        ...
        totalPickupStations: pickupStationStats?.total || 0,
      ...
      if (stats.totalRegistrars === 0) {
        // Registrar initialization is normal for new zones
      }
      return stats;
    } catch (error) {
