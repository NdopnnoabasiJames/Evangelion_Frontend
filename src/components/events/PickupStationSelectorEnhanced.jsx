    ...
    setSuccess(null);try {
      ...
      const pickupStationsData = selectedStations.map(stationId => {
        ...
        if (station.departureTime) {
          ...
          // For multiday events, use the start date
          const eventDate = event.eventType === 'multi-day' 
      ...
      console.log('🔍 [Frontend Enhanced] Assigning pickup stations:', {
        endpoint: API_ENDPOINTS.EVENTS.ASSIGN_PICKUP_STATIONS,
        eventId: event._id,
        stationsCount: pickupStationsData.length,
        stations: pickupStationsData
      });
      ...
      console.log('🔍 [Frontend Enhanced] Assignment result:', result);      // Check if there was an API error first
      if (apiError) {
        setError(typeof apiError === 'string' ? apiError : JSON.stringify(apiError));
        return;
      }

      // Verify that the assignment was actually successful
      if (result && (result.success || result.pickupStations || result.message?.includes('success'))) {
        ...
        console.error('🚨 [Frontend Enhanced] Assignment validation failed:', {
          result,
          hasSuccess: !!result?.success,
          hasPickupStations: !!result?.pickupStations,
          hasSuccessMessage: result?.message?.includes('success')
        });
        setError('Assignment failed - no stations were saved. Please try again.');
        return;
      }    } catch (error) {
      setError(`Error: ${error.message}`);
    } finally {
