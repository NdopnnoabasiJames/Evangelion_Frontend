...
const EventDelegation = ({ events, userRole, onDelegationComplete, isReadOnly = false }) => {
  ...
  // Filter to only show upcoming/active events (not past events)
  const filterUpcomingEvents = (eventsList) => {
    const today = new Date();
    today.setHours(0, 0, 0, 0); // Set to start of today for comparison
    
    const eventsArray = Array.isArray(eventsList) ? eventsList : 
                       Array.isArray(eventsList?.data) ? eventsList.data : [];
    
    return eventsArray.filter(event => {
      // Use the utility function to check if event is upcoming
  ...
  if (upcomingEvents.length === 0) {
    return (
      <EmptyState 
        icon="bi-calendar-x"
        title="No Upcoming Events Pending Delegation"
        description="There are no upcoming events that need zone selection. Past events are automatically hidden."
      />
    );
  }
