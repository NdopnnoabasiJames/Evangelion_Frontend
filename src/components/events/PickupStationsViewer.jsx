...
const PickupStationsViewer = ({ userRole }) => {
  ...
  const loadBranches = async () => {
    ...
    try {
      // Try to use the correct endpoint for branches by state
      const branchesResponse = await fetchBranches(`/api/branches?stateId=${selectedState}`);
      ...
      try {
        ...
        console.error('Failed to load branches with fallback:', fallbackError);
        setBranches([]);
      }
    }
  };
  ...
  const filterStations = () => {
    ...
    if (selectedState) {
      filtered = filtered.filter(station => 
        station.stateId?._id === selectedState || 
        station.stateId === selectedState ||
        station.branchId?.stateId?._id === selectedState ||
        station.branchId?.stateId === selectedState
      );
    }

    if (selectedBranch) {
      filtered = filtered.filter(station => 
        station.branchId?._id === selectedBranch || 
        station.branchId === selectedBranch
      );
    }
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            {(userRole === 'super_admin' || userRole === 'super_me') ? 'View all pickup stations across states and branches' : 'Manage pickup stations for events'}
          </p>
        </div>
                ...
                <div className="card-body">
                  ...
                  <div className="mb-3">
                    <small className="text-muted d-block">
                      ...
                      <strong>Zone:</strong> {station.zoneId?.name || 'N/A'}
                    </small>
                    <small className="text-muted d-block">
                      <i className="bi bi-diagram-3 me-1"></i>
                      <strong>Branch:</strong> {station.branchId?.name || 'N/A'}
                    </small>
                    <small className="text-muted d-block">
                      <i className="bi bi-map me-1"></i>
                      <strong>State:</strong> {station.branchId?.stateId?.name || station.stateId?.name || 'N/A'}
                    </small>
                  </div>
