...
const PageHeader = ({ 
  ...
  // Return Bootstrap font-size utility classes so the <h1> can scale responsively
  const getTitleSize = () => {
    switch (size) {
    ...
    <>
      ...
      <div className={headerClasses}>
        <div>
          ...
          {subtitle && (() => {
            // Provide a compact, truncated subtitle for mobile and the full subtitle for md+
            const shortSubtitle = String(subtitle).split(' - ')[0].split(',')[0];
            return (
              <>
                {/* Mobile: shortened & ellipsized */}
                <p
                  className="text-muted mb-0 mt-1 d-block d-md-none"
                  style={{
                    maxWidth: '180px',
                    overflow: 'hidden',
                ...
                <p className="text-muted mb-0 mt-1 d-none d-md-block">
                  {subtitle}
                </p>
              </>
            );
...
export const HeaderConfigurations = {
  ...
  dashboard: (role, name, onRefresh, stateInfo = null, branchInfo = null) => {
    ...
    if (role === 'state_admin' && stateInfo) {
      ...
      title = `${branchInfo.name} Branch Pastor Dashboard`;
    ...
    return {
      title,
      subtitle
      // No actions (removes Refresh button)
    };
  }
};
