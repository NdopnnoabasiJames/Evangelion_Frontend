...
const PickupStationsTab = ({ 
  ...
  // Extract available zones with counts
  const availableZones = React.useMemo(() => {
    if (!pickupStations || !Array.isArray(pickupStations)) return [];
    
    const zoneMap = new Map();
    pickupStations.forEach(station => {
      const zoneName = station.zoneId?.name;
      if (zoneName) {
  ...
  const filteredStations = React.useMemo(() => {
    ...
    let filtered = pickupStations.filter(station => {
      ...
      // Zone filter logic (OR logic for multiple selected zones)
      const matchesZone = selectedZones.length === 0 || 
        selectedZones.includes(station.zoneId?.name);
      
      return hasName && matchesSearch && matchesZone;
    });
    ...
    return filtered;
  }, [pickupStations, searchTerm, selectedZones, sortField, sortDirection]);
    ...
    <div className="pickup-stations-tab">
        ...
        <div className="col-md-4">
          <div className="dropdown">
            ...
            <ul className="dropdown-menu w-100" style={{ maxHeight: '300px', overflowY: 'auto' }}>
              ...
              {availableZones.length > 0 && <li><hr className="dropdown-divider" /></li>}
              {availableZones.map((zone) => (
                <li key={zone.name}>
                  <button
                    className={`dropdown-item d-flex justify-content-between align-items-center ${
                      selectedZones.includes(zone.name) ? 'active' : ''
                    }`}
                    onClick={() => handleZoneFilter(zone.name)}
                  >
                    <span>
                      <i className={`bi ${
                        selectedZones.includes(zone.name) ? 'bi-check-square' : 'bi-square'
                      } me-2`}></i>
                      {zone.name}
                    </span>
      ...
      {filteredStations.length === 0 ? (
        <div className="text-center py-5">
          ...
          {(searchTerm || selectedZones.length > 0) && (
            <p className="text-muted">
              Try adjusting your search terms or zone filters
            </p>
          )}
        </div>
      ) : (
