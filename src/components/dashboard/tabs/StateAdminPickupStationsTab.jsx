...
const StateAdminPickupStationsTab = () => {
  const columns = [
    {
      field: 'location',
      label: (
    ...
    },
    {
      ...
      render: (value, station) => (
        <div className="d-flex align-items-center">
          ...
          <span>{station.branchId?.name || '-'}</span>
        </div>
      )
    }
  ];
  ...
  return (
    <PickupStationsTab
      title="State Pickup Stations"
      endpoint={API_ENDPOINTS.PICKUP_STATIONS.STATE_ADMIN_LIST}
      columns={columns}
      showZone={true}
      showBranch={true}
      showState={false}
...
export default StateAdminPickupStationsTab;
