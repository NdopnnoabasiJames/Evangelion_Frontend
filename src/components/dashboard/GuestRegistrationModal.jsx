    ...
    const loadEvents = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Additional client-side filtering to ensure no expired events
          ...
          const upcomingEvents = eventsArray.filter(event => {
            ...
            if (event.eventType === 'multi-day') {
              if (!event.endDate) return true; // Include if no endDate
              const eventEndDate = new Date(event.endDate);
              return eventEndDate >= currentDate;
            }
            
            // For single-day events, check the date
            if (!event.date) return true; // Include if no date
  ...
  const handleGuestRegistration = async (e) => {
    ...
    try {
      const requestBody = {
        ...
        isNewConvert: guestForm.isNewConvert,
        isElderly: guestForm.isElderly
      };
              ...
              <div className="row">
                ...
                <div className="col-md-6 mb-3">
                  ...
                  >
                    ...
                    {events.map(event => {
                      // Format date display for dropdown
                      ...
                      if (event.eventType === 'multi-day') {
                        if (event.startDate && event.endDate) {
                          ...
                          dateDisplay = `${new Date(event.startDate).toLocaleDateString()} (Start)`;
                        } else if (event.endDate) {
                          dateDisplay = `${new Date(event.endDate).toLocaleDateString()} (End)`;
                        }
                      } else if (event.date) {
                      ...
                      return (
                        <option key={event._id} value={event._id}>
                          {event.name || event.title || 'Unnamed Event'} - {dateDisplay}
                        </option>
                      );
                    })}
                  </select>
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check">
                    <input
                      className="form-check-input"
                      type="checkbox"
                      checked={guestForm.isElderly}
                      onChange={(e) => setGuestForm({ ...guestForm, isElderly: e.target.checked })}
                      id="elderlyCheck"
                    />
                    <label className="form-check-label" htmlFor="elderlyCheck">
                      Elderly
                    </label>
                  </div>
                </div>
