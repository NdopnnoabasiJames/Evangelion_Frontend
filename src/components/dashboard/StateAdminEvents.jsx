...
const StateAdminEvents = () => {
  ...
  const handleEditEvent = (event) => {
    // State admins can only edit their own events, not super admin events
    if (event.creatorLevel === 'state_admin') {
      setEditingEvent(event);
      setEventActiveTab('edit');
    }
  };

  // Function to determine if an event can be edited by state admin
  const canEditEvent = (event) => {
    return event.creatorLevel === 'state_admin';
  };
          ...
          <TabPane tabId="list" title="All Events">
            ...
            <div className="mb-3">
              ...
              <p className="text-muted">
                ...
                available in your state (which can only be delegated, not edited) and your own events that can be further delegated to zones.
              </p>
              <div className="alert alert-info">
                <i className="bi bi-info-circle me-2"></i>
                <strong>Note:</strong> Super admin events can only be delegated to branches. To edit event details, 
                you can only modify events you created yourself from the "All Events" tab.
              </div>
            </div>
            <div className="events-container">
              <EventDelegation 
