...
const BranchesManagement = ({ isReadOnly = false }) => {
  ...
  const loadBranches = async () => {
    try {
      ...
      const endpoint = user?.role === ROLES.SUPER_ADMIN || user?.role === ROLES.SUPER_ME || user?.role === ROLES.STATE_ADMIN
  ...
  useEffect(() => {
    ...
    if (filters.search) {
      filtered = filtered.filter(branch =>
        ...
        (branch.branchAdmins && branch.branchAdmins.some(admin => 
    ...
    if (filters.adminFilter !== 'all') {
      if (filters.adminFilter === 'has-admin') {
        filtered = filtered.filter(branch => 
          branch.branchAdmin || (branch.branchAdmins && branch.branchAdmins.length > 0)
        );
  ...
  const handleExportBranches = () => {
    ...
    const exportData = filteredBranches.map(branch => ({
      ...
      branchAdmin: branch.branchAdmins && branch.branchAdmins.length > 0
        ? branch.branchAdmins.map(admin => admin.name).join(', ')
        : branch.branchAdmin?.name || 'No pastor assigned',
      adminEmail: branch.branchAdmins && branch.branchAdmins.length > 0
        ? branch.branchAdmins.map(admin => admin.email).join(', ')
    ...
    <div className="container-fluid">
      <div className="row">
        ...
        <div className="card-body">
          {/* Sub-tabs for Super Admin only: Pending Approval and Rejected */}
          ...
          {activeTab === 'pending' && user?.role === ROLES.SUPER_ADMIN && (
            <>
              <div className="d-flex justify-content-between align-items-center mb-3">
                          ...
                          <td>
                            <div className="btn-group" role="group">
                              <button
                                ...
                                onClick={() => handleApproveBranch(branch._id)}
                              ...
                              <button
                                ...
                                onClick={() => handleRejectBranch(branch._id)}
                                title="Reject Branch"
                              >
          ...
          {activeTab === 'rejected' && user?.role === ROLES.SUPER_ADMIN && (
            <>
              <div className="d-flex justify-content-between align-items-center mb-3">
