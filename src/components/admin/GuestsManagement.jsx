...
const GuestsManagement = () => {
  ...
  // Check if user is a branch admin - hide branch column since they can only see their own branch
  // Check both role and currentRole, and handle both uppercase and lowercase variations
  const isBranchAdmin = [
    'BRANCH_ADMIN', 'BRANCH_ME', 
    'branch_admin', 'branch_me'
  ].includes(user?.role) || [
    'BRANCH_ADMIN', 'BRANCH_ME', 
    'branch_admin', 'branch_me'
  ...
  // Hide branch column for branch admins since they can only see their own branch anyway
  const shouldHideBranch = isBranchAdmin;
  ...
  useEffect(() => {
    ...
    if (filters.statusFilter !== 'all') {
      if (filters.statusFilter === 'first timer') {
        filtered = filtered.filter(guest => guest.firstTimer === true);
      } else if (filters.statusFilter === 'C/assimilation') {
        filtered = filtered.filter(guest => guest.commenceAssimilation === true);
      } else if (filters.statusFilter === 'checked_in') {
        filtered = filtered.filter(guest => guest.checkedIn === true);
      } else if (filters.statusFilter === 'elderly') {
        filtered = filtered.filter(guest => guest.isElderly === true);
      } else {
    ...
    if (filters.registrationTypeFilter !== 'all') {
      filtered = filtered.filter(guest => guest.registrationType === filters.registrationTypeFilter);
    }
  ...
  const handleExportGuests = () => {
    const columns = [
      ...
      // Only include branch column for non-branch admins
      ...(shouldHideBranch ? [] : [{ key: 'branch', label: 'Branch' }]),
    ...
    const exportData = filteredGuests.map(guest => ({
      ...
      // Only include branch for non-branch admins
      ...(shouldHideBranch ? {} : { branch: guest.branch?.name || '' }),
  ...
  const getStatusBadgeClass = (guest) => {
    if (guest.firstTimer) return 'bg-primary';
    if (guest.commenceAssimilation) return 'bg-info';
    if (guest.checkedIn) return 'bg-success';
  ...
  const formatStatusText = (guest) => {
    if (guest.firstTimer) return 'First Timer';
    if (guest.commenceAssimilation) return 'C/assimilation';
    if (guest.checkedIn) return 'Checked In';
    
    switch (guest.status) {
      case 'no_show': return 'No Show';
      default: return guest.status.charAt(0).toUpperCase() + guest.status.slice(1);
    }
  };
            ...
            <div className="card border-0 bg-light">
              <div className="card-body">
                <div className="row g-3">
                  ...
                  <div className="col-md-2">
                    ...
                    >
                      ...
                      <option value="C/assimilation">C/assimilation</option>
                  ...
                  <div className="col-md-2">
                    ...
                    <select
                      className="form-select"
                      value={filters.registrationTypeFilter}
                      onChange={(e) => handleFilterChange('registrationTypeFilter', e.target.value)}
                    >
                ...
                <div className="row g-3 mt-2">
                  {/* Branch Filter - Hidden for branch admins since they can only see their own branch */}
                  {!shouldHideBranch && (
          ...
          <div>
            ...
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              <div className="d-flex align-items-center gap-2">
                {(filters.search || filters.eventFilter !== 'all' || filters.statusFilter !== 'all' || filters.transportFilter !== 'all' || filters.registrationTypeFilter !== 'all' || filters.branchFilter !== 'all' || filters.stateFilter !== 'all') && (
                  ...
                  {filteredGuests.map(guest => {
                    return (
                      <tr key={guest._id}>
                        ...
                        <td>
                          <span className={`badge ${guest.firstTimer ? 'bg-info text-dark' : 'bg-light text-dark'}`}>
                        ...
                        <td>
                          <span className={`badge ${guest.isElderly ? 'bg-purple text-dark' : 'bg-light text-dark'}`}>
                            {guest.isElderly ? 'Yes' : 'No'}
                          </span>
                        </td>
                        <td>
                          <span className={`badge ${getStatusBadgeClass(guest)} text-white`}>
                            {formatStatusText(guest)}
                          </span>
                        </td>
                      </tr>
                    );
