...
import { sessionEventHandlers, alertUtils } from '../utils/alertUtils';
...
export const AuthProvider = ({ children }) => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(false);  useEffect(() => {
    ...
    const cleanupSessionListener = sessionEventHandlers.setupSessionListener();
    
    // Expose logout function globally for session expiration handling
  ...
  const login = async (credentials) => {
    try {
      ...
      try {
        ...
        alertUtils.showLoginSuccess(normalizedUser.firstName || normalizedUser.name);
        ...
        // Show success notification with fallback data
        alertUtils.showLoginSuccess(userData.firstName || userData.name);
  ...
  const logout = (showNotification = true) => {
    ...
    // Show logout notification if requested
    if (showNotification) {
      alertUtils.showLogoutSuccess();
    }
  };
  const hasRole = (role) => {
